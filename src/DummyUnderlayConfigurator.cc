...
/**
 * @file DummyUnderlayConfigurator.cc
...
Define_Module(DummyUnderlayConfigurator);

using namespace std;

DummyUnderlayConfigurator::~DummyUnderlayConfigurator()
{
...
void DummyUnderlayConfigurator::initializeUnderlay(int stage)
{
    if (stage != MAX_STAGE_UNDERLAY)
        return;
...
TransportAddress* DummyUnderlayConfigurator::createNode(NodeType type,
                                                         bool initialize)
{
    Enter_Method_Silent();
    return new TransportAddress();// NULL;
    /*
...
uint32_t DummyUnderlayConfigurator::parseCoordFile(const char* nodeCoordinateSource)
{
...
void DummyUnderlayConfigurator::preKillNode(NodeType type, TransportAddress* addr)
{
    Enter_Method_Silent();
/*
...
void DummyUnderlayConfigurator::migrateNode(NodeType type, TransportAddress* addr)
{
    Enter_Method_Silent();
    /*
...
void DummyUnderlayConfigurator::handleTimerEvent(cMessage* msg)
{
    Enter_Method_Silent();
    /*
    ...
    if (info != NULL) {
        ...
        throw cRuntimeError("DummyUnderlayConfigurator: Trying to kill "
...
void DummyUnderlayConfigurator::setDisplayString()
{
    // Updates the statistics display string.
    char buf[80];
    sprintf(buf, "%i overlay terminals", 0);//overlayTerminalCount);
    getDisplayString().setTagArg("t", 0, buf);
}

void DummyUnderlayConfigurator::finishUnderlay()
{
    // statistics
...
IPvXAddress DummyUnderlayConfigurator::migrateNode(NodeType type, IPvXAddress addr,
...
double DummyUnderlayConfigurator::getDistance(const BaseLocation& IDa, const BaseLocation& IDb){
    //NodeRecord a = dynamic_cast<const NodeRecord&>(IDa);
    //NodeRecord b = dynamic_cast<const NodeRecord&>(IDb);
    return 1;//sqrt( pow((a.coords[0] - b.coords[0]), 2)
              //   + pow((a.coords[1] - b.coords[1]), 2) );
}

BaseLocation* DummyUnderlayConfigurator::getNearLocation(const BaseLocation& ID, double radius){
    //return new BaseLocation();//NULL;/*
...
BaseLocation* DummyUnderlayConfigurator::getLocation(IPvXAddress addr){
    NodeRecord* ret = new NodeRecord();
        return ret;
    /*
